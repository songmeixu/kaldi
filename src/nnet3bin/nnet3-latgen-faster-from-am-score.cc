...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        "Generate lattices from am score using nnet3 neural net model.\n"
            "Usage: nnet3-latgen-faster-from-am-score [options] <nnet-in>"
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !loglikes_reader.Done(); loglikes_reader.Next()) {
          ...
          DecodableMatrixScaledMapped decodable(trans_model, loglikes, 1.0);
