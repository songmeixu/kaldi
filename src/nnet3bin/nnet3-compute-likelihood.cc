...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Propagate the features through neural network model (with prior applyed) "
            "and write the output.\n"
            "If --apply-exp=true, apply the Exp() function to the output "
            "before writing it out.\n"
            "\n"
            "Usage: nnet3-compute-likelihood [options] <model-in> <features-rspecifier> <matrix-wspecifier>\n"
            " e.g.: nnet3-compute-likelihood final.mdl scp:feats.scp ark:nnet_prediction.ark\n"
    ...
    po.Register("apply-exp", &apply_exp, "If true, apply exp function to "
        "output");
    po.Register("apply-prior", &apply_prior, "If true, apply exp function to "
        "output");
