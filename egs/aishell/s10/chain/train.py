...
from libs.nnet3.train.dropout_schedule import get_dropout_proportions
...
def get_objf(
    ...
    nnet_output = nnet_output.contiguous().view(-1, nnet_output.shape[-1])

    # at this point, xent_output is: [N, T, C]
    xent_output = xent_output.permute(1, 0, 2)
    # at this point, xent_output is: [T, N, C]
    xent_output = xent_output.contiguous().view(-1, xent_output.shape[-1])
    objf_l2_term_weight = criterion(
        opts, den_graph, supervision, nnet_output, xent_output
    )
...
def train_one_epoch(
    ...
    for batch_idx, (pseudo_epoch, batch) in enumerate(dataloader):
        ...
        data_fraction = (pseudo_epoch + 1 + current_epoch * len(dataloader)) / (
            len(dataloader) * num_epochs
        )
        _, dropout = get_dropout_proportions(dropout_schedule, data_fraction)[0]
        ...
        if batch_idx % 100 == 0:
            logging.info(
                ...
                "frames, current batch average objf: {:.6f} over {} frames, epoch {}".format(
                    device.index,
                    batch_idx,
                    pseudo_epoch,
                    len(dataloader),
...
def process_job(learning_rate, device_id=None, local_rank=None):
    """
    Start a process to finish a training process.
    It could be a process of local device or Grid Workers.

    :param learning_rate: the starting learning rate
    :param device_id: the device ID to be used
    :param local_rank: the rank of this job
    :return: none
    """
    args = get_args()
    if local_rank is not None:
        setup_logger(
            "{}/logs/log-train-rank-{}".format(args.dir, local_rank), args.log_level,
        )
    else:
    ...
    try:
        for epoch in range(start_epoch, args.num_epochs):
            ...
            if dataloader.sampler and isinstance(
                dataloader.sampler, DistributedSampler
            ):
                dataloader.sampler.set_epoch(epoch)

            objf = train_one_epoch(
            ...
            epoch_info_filename = os.path.join(args.dir, "epoch-{}-info".format(epoch))
            save_training_info(
        ...
        model_path = os.path.join(args.dir, "epoch-{}-interrupted.pt".format(epoch))
