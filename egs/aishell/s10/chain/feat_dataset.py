...
def get_feat_dataloader(
    ...
    collate_fn = FeatDatasetCollateFunc(
...
class FeatDatasetCollateFunc:
    ...
    def __call__(self, batch):
        ...
        for b in batch:
            ...
            output_len = (
                feat_len + self.frame_subsampling_factor - 1
            ) // self.frame_subsampling_factor
            output_len_list.append(output_len)
            # now add model left and right context
            feat = _add_model_left_right_context(
                feat, self.model_left_context, self.model_right_context
            )

            # now we split feat to chunk, then we can do decode by chunk
            input_num_frames = (
                feat.shape[0] - self.model_left_context - self.model_right_context
            )
            for i in range(0, output_len, subsampled_frames_per_chunk):
                ...
                last_output = min(
                    input_num_frames,
                    first_output
                    + (subsampled_frames_per_chunk - 1) * self.frame_subsampling_factor,
                )
                first_input = first_output
                last_input = (
                    last_output + self.model_left_context + self.model_right_context
                )
        ...
        padded_feat = pad_sequence(
            [torch.from_numpy(feat).float() for feat in feat_list], batch_first=True,
        )

        assert (
            sum([math.ceil(l / subsampled_frames_per_chunk) for l in output_len_list])
            == padded_feat.shape[0]
        )
