...
def load_checkpoint(filename, model):
    ...
    if not list(model.state_dict().keys())[0].startswith("module.") and list(
        checkpoint["state_dict"]
    )[0].startswith("module."):
        # the checkpoint was saved by DistributedDataParallel
        logging.info("load checkpoint from DistributedDataParallel")
...
def save_checkpoint(filename, model, epoch, learning_rate, objf, local_rank=0):
    ...
    logging.info(
        "Save checkpoint to {filename}: epoch={epoch}, "
        "learning_rate={learning_rate}, objf={objf}".format(
            filename=filename, epoch=epoch, learning_rate=learning_rate, objf=objf
        )
    )
    checkpoint = {
