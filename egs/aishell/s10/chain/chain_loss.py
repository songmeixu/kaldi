...
class KaldiChainObjfFunction(Function):
    ...
    @staticmethod
    def forward(
        ctx, opts, den_graph, supervision, nnet_output_tensor, xent_out_unused_tensor,
    ):
        """
        ...
        if (
            g_nnet_output_deriv_tensor is None
            or g_nnet_output_deriv_tensor.shape != nnet_output_tensor.shape
        ):
        ...
        chain.ComputeChainObjfAndDeriv(
            opts=opts,
            den_graph=den_graph,
        ...
        ctx.save_for_backward(g_nnet_output_deriv_tensor, g_xent_output_deriv_tensor)
