...
class NnetChainExampleDatasetCollateFunc:
    def __init__(self, egs_left_context, egs_right_context, frame_subsampling_factor):
        """
    ...
    def __call__(self, batch):
        ...
        for b in batch:
            ...
            for i in range(batch_size):
                ...
                if len(eg.inputs) > 1:
                    repeat_ivector = torch.from_numpy(ivectors[i]).repeat(
                        feat.shape[0], 1
                    )
                    feat = torch.cat(
...
def _test_nnet_chain_example_dataset():
    ...
    dataloader = DataLoader(dataset, batch_size=1, num_workers=0, collate_fn=collate_fn)
    for b in dataloader:
        key_list, feature_list, supervision_list = b
        assert (
            feature_list[0].shape == (128, 204, 129)
