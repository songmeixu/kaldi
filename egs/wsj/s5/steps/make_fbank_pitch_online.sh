...
required="$scp $fbank_config $online_pitch_config"

for f in $required; do
  if [ ! -f $f ]; then
    echo "make_fbank_pitch_online.sh: no such file $f"
    exit 1;
  fi
done
...
for n in $(seq $nj); do
  ...
  utils/create_data_link.pl $fbank_pitch_dir/raw_fbank_online_pitch_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_fbank_pitch_${name}.JOB.log \
    ...
    copy-feats --compress=$compress ark:- \
      ark,scp:$fbank_pitch_dir/raw_fbank_online_pitch_$name.JOB.ark,$fbank_pitch_dir/raw_fbank_online_pitch_$name.JOB.scp \
     || exit 1;
...
for n in $(seq $nj); do
  cat $fbank_pitch_dir/raw_fbank_online_pitch_$name.$n.scp || exit 1;
done > $data/feats.scp
