...
if $use_graphs; then
  ...
  tra="ark:local/sym2int++.pl -f 2- $lang/phones.txt $sdata/JOB/phn_dur.ark|";
  # We could just use gmm-align in the next line, but it's less efficient as it compiles the
  # training graphs one by one.
  $cmd JOB=1:$nj $dir/log/align.JOB.log \
    compile-train-graphs-from-supervision --read-disambig-syms=$lang/phones/disambig.int \
      $dir/tree $dir/final.mdl "$tra" ark: ark:- \| \
    gmm-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam --careful=$careful "$mdl" ark:- \
      "$feats" "ark,t:$dir/ali.JOB" "ark,t:$dir/score.JOB" || exit 1;
