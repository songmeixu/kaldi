...
if [ $stage -le -2 ]; then
  ...
  stddev2=`perl -e "print 1.0/sqrt($pnorm_1st_input_dim);"`
...
AffineComponentPreconditionedOnline input-dim=$lda_dim output-dim=$pnorm_1st_input_dim $online_preconditioning_opts learning-rate=$initial_learning_rate param-stddev=$stddev2 bias-stddev=$bias_stddev
PnormComponent input-dim=$pnorm_1st_input_dim output-dim=$pnorm_output_dim p=$p
