...
if [ ! -d $s1 ]; then
  ...
  need_to_split=false
  for f in utt2spk spk2utt spk2warp feats.scp ali.scp am_like.scp text wav.scp cmvn.scp spk2gender \
...
for f in feats.scp text vad.scp ali.scp am_like.scp utt2lang $maybe_wav_scp; do
  if [ -f $data/$f ]; then
